...
class ProductRequestModel(BaseModel):
    ...
    product_type: ProductTypes = Field(
        title="Product type",
        default=ProductTypes.DESKTOP_PC
    )
    base64_image: Optional[str] = Field(
        title="Base64-encoded image",
        # Prepare a base64-encoded image and paste here.
...
class ProductUpdateRequestModel(BaseModel):
    """
    Product request fields for updating product information. 
    """
    ...
    product_type: Optional[ProductTypes] = Field(
        title="Product type",
        example=ProductTypes.DESKTOP_PC
    )
